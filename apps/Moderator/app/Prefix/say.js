...
class Say extends PrefixCommand {
    ...
    async run(client, message, args) {
        ...
        > <a:146_yldrm:948840799343353877> Taglı sayısı: \`${message.guild.members.cache.filter(m => client.config.tags.some(tag => m.user.username.includes(tag)) || client.config.dis === m.user.discriminator).size}\` (${message.guild.members.cache.filter(m => m.roles.cache.has(client.data.roles["cmd-crew"])).size} yetkili)
        > <:146_1463:948715248477675560> Anlık ses: \`${message.guild.members.cache.filter(m => m.voice && m.voice.channel).size}\`
        `);
    }
}
